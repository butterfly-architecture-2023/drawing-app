...
class Plane : NSObject, MakerablePlane, TouchablePlane, ObjectSelectablePlane, SyncServiceUsecase, LoginServiceUsecase {
    enum Notifications {
    ...
    func login() {
        NotificationCenter.default.post(name: UsecaseNotifications.requestToLogin, object: self, userInfo: [:])
    }

    func successToLogin(with id: String) {
        self.id = id
        NotificationCenter.default.post(name: UsecaseNotifications.requestToSync, object: self, userInfo: ["data" : self.objects])
    }
