...
class SquareManagerImpl: SquareManager {
    ...
    func makeSquare(in rect: CGRect) -> Drawable? {
        ...
        let points: [CGPoint] = [
            ...
            CGPoint(x: minX + Constants.squareWidth, y: minY),
            CGPoint(x: minX + Constants.squareWidth, y: minY + Constants.squareHeight),
            CGPoint(x: minX, y: minY + Constants.squareHeight)
        ]
